...
% Where you can change the stimulus image files, etc. We are loading in 20 where the 1st 10 
% are black and the second 10 are same-shape white
% The simple shapes are preallocated in another file that is loaded in at
% the top
nimages = 10;
image_tex=[];
for i=1:nimages
    for j=1:length(colors)
        image_tex(i,j)=Screen('MakeTexture',w,imageArrayColor(:,:,:,i,j));
        image_tex(i+10,j)=Screen('MakeTexture',w,imageArrayBlack(:,:,:,i));
    end
end
% Make the black square
square_tex=Screen('MakeTexture',w,imageSquare);
...
%             y_rand = 50-10*randi(10);
        ...
        %Screen('Flip',w);
        Screen('DrawTexture',w,square_tex, [], destrect_square(1,:));
        Screen('DrawTexture',w,square_tex, [], destrect_square(2,:));
        Screen('DrawTexture',w,square_tex, [], destrect_square(3,:));
        Screen('DrawTexture',w,square_tex, [], destrect_square(4,:));
        Screen('Flip',w);
        ...
        if timingTester == 0
            while ~keyisdown && time < 4
                ...
                if time < 1
                    Screen('DrawTexture',w,texArray(probe,2),[],destrect(target_pos,:));
                else
                    Screen('DrawTexture',w,blank_tex,[],rect);
                end
                Screen('DrawTexture',w,fix_tex,[],fix_rect);

...
% rawdata(n,8) = trial presentation order
...
repetitions = 20;
varList = repmat(fullyfact([nGrouped nOldNew nProbeFreqLoc]),[repetitions,1]);
...
datadir = '/Users/C-Lab/Google Drive/Lab Projects/Marians Stuff/R15 wmGrouping/Freq_tag_code/data/';
...
image_tex=[];
for i=1:10
    images=imread(sprintf('%s%d%s','/Users/C-Lab/Google Drive/Lab Projects/Marians Stuff/R15 wmGrouping/Freq_tag_code/shape_stimuli/',i,'.tif'));
    images=images(:,:,1);
    images2=images;
    ...
    image_tex(i)=Screen('MakeTexture',w,images);
    image_tex(i+10)=Screen('MakeTexture',w,images2);
end

images=imread(sprintf('%s%d%s','/Users/C-Lab/Google Drive/Lab Projects/Marians Stuff/R15 wmGrouping/Freq_tag_code/shape_stimuli2/',1,'.tif'));
images=images(:,:,1);
images2=images;
...
while missed
    for n = trialOrder
        ...
        groupedIdx = varList(trial,1); % 1=grouped 2= not grouped 3=non grouped
        rawdata(n,1) = groupedIdx;
        oldNewIdx = varList(trial,2); % 1=old 2=new
        rawdata(n,2) = oldNewIdx;
        probeLocIdx = varList(trial,3); % which is probed: 1-4 = F1-F4
        rawdata(n,3) = probeLocIdx;
        ...
        switch groupedIdx
            case 1
                ...
                if target_pos == 1 || target_pos == 3
                    ...
                    tex(1,1) = im(whichImage);
                    tex(1,2) = im(whichImage)+10;
                    tex(2,1) = im(2);
                    tex(2,2) = im(2)+10;
                    tex(3,1) = im(whichImage);
                    tex(3,2) = im(whichImage)+10;
                    tex(4,1) = im(4);
                    tex(4,2) = im(4)+10;
                elseif target_pos == 2 || target_pos == 4
                    ...
                    tex(1,1) = im(1);
                    tex(1,2) = im(1)+10;
                    tex(2,1) = im(whichImage);
                    tex(2,2) = im(whichImage)+10;
                    tex(3,1) = im(3);
                    tex(3,2) = im(3)+10;
                    tex(4,1) = im(whichImage);
                    tex(4,2) = im(whichImage)+10;
                end
        ...
        %Screen('Flip',w);
        Screen('DrawTexture',w,image_tex(nimages+1), [], destrect_square(1,:));
        Screen('DrawTexture',w,image_tex(nimages+1), [], destrect_square(2,:));
        Screen('DrawTexture',w,image_tex(nimages+1), [], destrect_square(3,:));
        Screen('DrawTexture',w,image_tex(nimages+1), [], destrect_square(4,:));
        Screen('Flip',w);
        ...
        Screen('DrawTextures',w,[image_tex(tex(1,flip1)),...
            image_tex(tex(2,flip2)),...
            image_tex(tex(3,flip3)),...
            image_tex(tex(4,flip4)),fix_tex,photo_tex],...
            [],[destrect(1,:);...
            destrect(2,:);...
            destrect(3,:);...
        ...
        if timingTester == 0
            while ~keyisdown && time < 4
                ...
                if time < 1
                    Screen('DrawTexture',w,image_tex(tex(probe,1)),[],destrect(target_pos,:));
                else
                    Screen('DrawTexture',w,blank_tex,[],rect);
                end
                Screen('DrawTexture',w,fix_tex,[],fix_rect);
                ...
                if keycode(buttonO)
                    rawdata(n,4) = 1; % responded it was old
                elseif keycode(buttonN)
                    rawdata(n,4) = 2; % responded it was new
                else
                    keyisdown = 0;
                end
            ...
            rawdata(n,5) = rawdata(n,2) == rawdata(n,4);
            
            rawdata(n,6) = probe; % which image was probed 1,2,3,4
            rawdata(n,7) = 1;    % 0 for no response 1 for responded
            
            if time >= 4
                
                %NetStation('Event', 'nore',GetSecs+.010  ,.010,  'tri#', i)
                rawdata(n,7) = 0;    % If they didn't respond record 0 here

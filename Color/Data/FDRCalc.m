...
for i=1:size(combinedIndexMean,2)
    ...
    for j=1:length(threshold)
        ...
        for z=1:size(combinedIndexMean,1)
            % Calculate the tval that corresponds to the threshold that is set
            [S, P(z,i,j), CI, T{z,i,j}] = ttest(combinedIndexMean(z,i,:),0,threshold(j));
        end
        ...
        % threshold given
        expectedFA(i,j) = threshold(j)*size(combinedIndexMean,1);
...
for i=1:length(closestThreshP)
    thresholdTStat(i) = abs(tinv((closestThreshP(i)/2),size(combinedIndexMean,3)-1));
end

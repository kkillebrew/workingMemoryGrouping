    ...
    for i=freqList
        for j=freqList
            if j == i
                        ...
                        load(sprintf('%s%s%s%s%d%s%d',inputFile,usableParticipants{a},'/Updated/FreqValues.updated/','noGrpFreq',i,'_',j));
%                                 load(sprintf('%s%s%s%s%s%d%s%d%s',inputFile,'FakeData/',usableParticipants{a},'/FreqValues.updated/','noGrpFreq',i,'_',j,'_noise'));
%                 load(sprintf('%s%s%s%s%s%d%s%d',inputFile,'FakeData/',usableParticipants{a},'/FreqValues/','noGrpFreq',i,'_',j));
                %                 load(sprintf('%s%s%s%s%d%s%d',inputFile,usableParticipants{a},'/FreqValues/','noGrpFreq',i,'_',j));
                                ...
                                load(sprintf('%s%s%s%s%d%s%d',inputFile,usableParticipants{a},'/Updated/FreqValues.updated/','grpFreq',i,'_',j));
%                                 load(sprintf('%s%s%s%s%s%d%s%d%s',inputFile,'FakeData/',usableParticipants{a},'/FreqValues.updated/','grpFreq',i,'_',j,'_noise'));
    ...
    for j=freqList
        for k=freqList
            if k == j
                ...
                noGroupTemp = eval(sprintf('%s%d','noGrpFreq',j,'_',k));
...
%     str = {'',sprintf('%s','2f1',testList{i},' + 2f2',testList{i}),''}; % cell-array method
    ...
    savefig(sprintf('%s',inputFile,'TestFigs/updatedIndices.fig'));
end

% % Average across the 5 different test freq combinations
% counter = 1;
% for i=1:size(finalIndex,2)/4
%     for j=1:size(finalIndex,1)
%         
%         finalIndexTest(j,i) = mean(finalIndex(j,counter:counter+3));
%         finalIndexTestSTE(j,i) = mean(finalIndexSTE(j,counter:counter+3));
%         
%     end
%     counter = counter + 4;
% end
% 
% figure(size(finalIndexTest,2)+1);
% set(gcf,'units','normalized','position',[0 0 1 1]);
% for i=1:size(finalIndexTest,2)
%     subplot(2,3,i)
%     bar(finalIndexTest(:,i))
%     hold on
%     errorbar([finalIndexTest(:,i)],[finalIndexTestSTE(:,i)],'k.');
%     str = {'',sprintf('%s','f1',testList{i},' + f2',testList{i}),''}; % cell-array method
%     xlabel('Electrode','FontSize',15);
%     ylabel('Index','FontSize',15);
%     title(str,'FontSize',15,'FontWeight','bold');
%     % set(gca,'ylim',[-1.2,1.2])
% end
% savefig(sprintf('%s',inputFile,'TestFigs/averageTestIndices.fig'));
...
save(sprintf('%s%s',inputFile,'/tvalAverage_updated'),'tvalAverage');
...
save(sprintf('%s%s',inputFile,'/finalIndex_updated'),'finalIndex','combinedIndexMean','usableParticipants')

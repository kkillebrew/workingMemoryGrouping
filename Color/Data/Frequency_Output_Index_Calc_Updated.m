...
iterationNumber = 1000; % INSERT ITERATION HERE
...
%                     (i+3+j+3) (i+3+i+3+j+3) (i+3+j+3+j+3) (i+3+i+3+j+3+j+3)];
                ...
                chosenFreq = [i+j i+i+j i+j+j i+i+j+j 7 9 10 12];
                
                % Load in the data
                group = cell2mat(struct2cell(load(sprintf('%s%s%s%d%s%d',inputFile,usableParticipants{a},'/grpPrb',i,'_',j))));
                groupFreqLength = size(group,1);
                
                noGroup = cell2mat(struct2cell(load(sprintf('%s%s%s%d%s%d',inputFile,usableParticipants{a},'/noGrp',i,'_',j))));
                noGroupFreqLength = size(noGroup,1);
                ...
                for m=1:iterationNumber
                    ...
                    if noGroupFreqLength >= groupFreqLength
                        % Randomly pick a subset of the no group trials
                        randNoGroup = randperm(noGroupFreqLength,groupFreqLength);
                        noGroupTemp = noGroup(randNoGroup,:,:);
                        groupTemp = group;
                    elseif noGroupFreqLength < groupFreqLength
                        % Randomly pick a subset of the no group trials
                        randGroup = randperm(groupFreqLength,noGroupFreqLength);
                ...
                freqGroupArraySpecAve = mean(freqGroupArraySpec,3);
                freqNoGroupArraySpecAve = mean(freqNoGroupArraySpec,3);
...
%                 clear (sprintf('%s%d%s%d','noGrpFreq',i,'_',j));
                ...
                clear freqNoGroupArrayFullAve freqGroupArrayFullAve freqGroupArray freqNoGroupArray freqGroupArrayFull freqNoGroupArrayFull groupAve noGroupAve
                clear freqGroupArraySpec freqNoGroupArraySpec freqGroupArraySpecAve freqNoGroupArraySpecAve indexValues
            end
        end
    end
...
save(sprintf('%s%s',inputFile,'/finalIndex_1000it_1'),'indexValuesPartAve','indexValuesPartSTE','indexValuesFreqAve')
